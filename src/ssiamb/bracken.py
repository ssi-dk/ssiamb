...
def parse_bracken_file(path: Path) -> List[BrackenSpecies]:
    ...
    try:
        ...
        if missing_columns:
            raise ValueError(f"Missing required columns in Bracken file: {missing_columns}")
        ...
        species_list = [BrackenSpecies.from_row(row) for _, row in species_df.iterrows()]
...
def select_species(
    species_list: List[BrackenSpecies],
    ...
    candidates = [
        ...
        if (sp.fraction_total_reads >= thresholds.min_frac and
            sp.new_est_reads >= thresholds.min_reads)
    ]
...
def select_species_from_file(
    path: Path,
    thresholds: Optional[BrackenThresholds] = None
) -> Optional[BrackenSelection]:
    """
