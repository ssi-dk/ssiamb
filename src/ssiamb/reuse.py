...
def check_compatibility(
    ...
    coverage_fraction = shared_length / total_fasta_length if total_fasta_length > 0 else 0.0
    ...
    total_length_diff = (
        ...
        if total_fasta_shared > 0 else 0.0
    )
    ...
    if per_contig_violations:
        violations.append(f"Per-contig length violations: {'; '.join(per_contig_violations)}")
...
def has_duplicate_flags(bam_path: Path) -> bool:
    ...
    try:
        ...
        raise CompatibilityError(f"Failed to check duplicates in BAM file {bam_path}: {e}") from e
