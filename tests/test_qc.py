...
from src.ssiamb.qc import (
    QCThresholds, QCWarning, check_qc_metrics, log_qc_warnings,
...
class TestQCLogging:
    ...
    def test_log_qc_warnings(self, caplog):
        ...
        assert any("QC warnings detected" in record.message for record in caplog.records)
        assert any("breadth_10x" in record.message or "0.750" in record.message for record in caplog.records)
    ...
    def test_log_no_warnings(self, caplog):
        ...
        assert len(caplog.records) >= 1
        assert any("passed warning thresholds" in record.message for record in caplog.records)
