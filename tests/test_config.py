...
class TestSsiambConfig:
    ...
    def test_load_defaults_from_file(self):
        ...
        with patch('src.ssiamb.config.Path.exists', return_value=True), \
             patch('src.ssiamb.config.SsiambConfig._load_yaml', return_value=defaults_content):
            config = SsiambConfig._load_defaults()
...
class TestSsiambConfigLoad:
    ...
    def test_load_with_user_config(self):
        ...
        try:
            with patch('src.ssiamb.config.SsiambConfig._load_defaults', return_value=defaults), \
                 patch('src.ssiamb.config.SsiambConfig._apply_env_overrides', side_effect=lambda x: x), \
                 patch('src.ssiamb.config.SsiambConfig._normalize_species_aliases', side_effect=lambda x: x):
    ...
    def test_load_with_nonexistent_user_config(self):
        ...
        with patch('src.ssiamb.config.SsiambConfig._load_defaults') as mock_defaults, \
             patch('src.ssiamb.config.SsiambConfig._apply_env_overrides', side_effect=lambda x: x), \
             patch('src.ssiamb.config.SsiambConfig._normalize_species_aliases', side_effect=lambda x: x):
            
            mock_defaults.return_value = {"thresholds": {}, "species_aliases": {}, "tools": {}, "output": {}}
            
            # Should not raise an error, just use defaults
            config = SsiambConfig.load(non_existent_path)
...
class TestConfigGetterMethods:
    ...
    def test_get_species_alias(self):
        ...
        config = SsiambConfig(
            thresholds={},
            species_aliases={"ecoli": "Escherichia coli", "staph": "Staphylococcus aureus"},
            tools={},
            output={}
        )
    ...
    def test_get_tool_setting(self):
        ...
        assert config.get_tool_setting("minimap2", "nonexistent", "default") == "default"
    ...
    def test_full_config_loading_workflow(self):
        ...
        try:
            with patch('src.ssiamb.config.SsiambConfig._load_defaults') as mock_defaults, \
                 ...
                 patch('src.ssiamb.refdir.normalize_species_name', side_effect=lambda x: x.lower()):
                ...
                assert config.get_tool_setting("minimap2", "preset") == "sr"
                assert config.get_tool_setting("samtools", "sort_mem") == "1G"  # From defaults
