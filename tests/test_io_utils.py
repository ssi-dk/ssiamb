...
class TestSampleNameInference:
    ...
    def test_infer_sample_name_complex_patterns(self):
        ...
        test_cases = [
            ...
            (Path("test_sample.R1.fastq.gz"), Path("test_sample.R2.fastq.gz"), "test_sample"),
        ]
        ...
        error_cases = [
            ...
            (Path("data_L001_R1_001.fastq"), Path("data_L001_R2_001.fastq")),  # Doesn't handle multiple suffixes
        ]
...
class TestTSVWriting:
    ...
    def test_write_tsv_summary_append_mode(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.tsv', delete=False) as tmp_file:
            ...
            tmp_file.write("sample\tmode\tmapper\tcaller\tdp_min\tmaf_min\tdp_cap\tdenom_policy\tcallable_bases\tgenome_length\tbreadth_10x\tambiguous_snv_count\tambiguous_snv_per_mb\tambiguous_indel_count\tambiguous_del_count\tref_label\tref_accession\tbracken_species\tbracken_frac\tbracken_reads\talias_used\treused_bam\treused_vcf\truntime_sec\ttool_version\n")
            tmp_file.write("existing\tself\tminimap2\tbbtools\t10\t0.1\t100\texclude_dups\t500\t500\t1.0\t5\t10.0\t0\t0\tE.coli|NC_000913\tNC_000913\tNA\t0.0\t0\tNA\tFalse\tFalse\t50.0\t1.0.0\n")
            tmp_file.flush()
