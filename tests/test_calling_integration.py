...
class TestCallingRealData:
    ...
    def test_caller_tool_availability(self):
        ...
        assert len(available_callers) > 0, "At least one variant caller should be available"
    ...
    def test_pre_mapped_bam_properties(self, pre_mapped_bam_path):
        ...
        assert pre_mapped_bam_path.stat().st_size > 100000, "BAM should be reasonably large"
    ...
    def test_bbtools_calling_pre_mapped_bam(self, pre_mapped_bam_path, real_reference_path):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert result.success, f"BBTools calling should succeed: {result.error_message}"
            ...
            with open(result.vcf_path) as f:
                content = f.read()
                assert content.startswith("##fileformat=VCF"), "VCF should have proper header"
                assert "#CHROM" in content, "VCF should have column headers"
            
            # Calling should complete in reasonable time
            assert calling_time < 300, f"BBTools calling took too long: {calling_time:.2f} seconds"
            
            print(f"BBTools calling completed in {calling_time:.2f} seconds")
            print(f"VCF size: {result.vcf_path.stat().st_size:,} bytes")
    
    def test_bcftools_calling_pre_mapped_bam(self, pre_mapped_bam_path, real_reference_path):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Validate calling results
            assert result.success, f"bcftools calling should succeed: {result.error_message}"
            assert result.vcf_path.exists(), "Output VCF should be created"
            assert result.vcf_path.stat().st_size > 0, "Output VCF should not be empty"
            ...
            # Calling should complete in reasonable time
            assert calling_time < 300, f"bcftools calling took too long: {calling_time:.2f} seconds"
            
            print(f"bcftools calling completed in {calling_time:.2f} seconds")
            print(f"VCF size: {result.vcf_path.stat().st_size:,} bytes")
    
    def test_end_to_end_mapping_and_calling(self, real_reference_path, real_reads_r1_path, real_reads_r2_path):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert call_result.success, f"End-to-end calling should succeed: {call_result.error_message}"
            assert call_result.vcf_path.exists(), "Final VCF should be created"
            assert call_result.vcf_path.stat().st_size > 0, "Final VCF should not be empty"
    ...
    def test_caller_comparison_pre_mapped_bam(self, pre_mapped_bam_path, real_reference_path):
        ...
        for caller in [Caller.BBTOOLS, Caller.BCFTOOLS]:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                results[caller.value] = {
                    ...
                    'call_time': call_time,
                    'vcf_size': result.vcf_path.stat().st_size if result.vcf_path.exists() else 0,
...
class TestCallingPerformance:
    ...
    def test_calling_performance_scaling(self, pre_mapped_bam_path, real_reference_path):
        ...
        assert speedup_ratio < 4.0, f"Performance variance too high: {speedup_ratio:.2f}x"
    
    def test_caller_tool_performance_comparison(self, pre_mapped_bam_path, real_reference_path):
