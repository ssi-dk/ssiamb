...
class TestMappingRealData:
    ...
    def test_real_reference_properties(self, real_reference_path):
        ...
        assert real_reference_path.stat().st_size > 1000000, "Reference should be reasonably large"
        
        # Check that the reference has proper FASTA format
        with open(real_reference_path) as f:
            first_line = f.readline().strip()
            assert first_line.startswith('>'), "Reference should start with FASTA header"
    ...
    def test_real_reads_properties(self, real_reads_r1_path, real_reads_r2_path):
        ...
        assert real_reads_r1_path.stat().st_size > 100000, "R1 reads should be reasonably large"
        assert real_reads_r2_path.stat().st_size > 100000, "R2 reads should be reasonably large"
    ...
    def test_minimap2_index_creation_real_data(self, real_reference_path):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert indexes_exist(temp_ref, Mapper.MINIMAP2), "Minimap2 index should be created"
            
            # Index creation should complete in reasonable time
            assert index_time < 30, f"Index creation took too long: {index_time:.2f} seconds"
            ...
            for index_file in index_files:
                ...
                assert index_file.stat().st_size > 1000, f"Index file {index_file} should not be empty"
    ...
    def test_bwa_mem2_index_creation_real_data(self, real_reference_path):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Should create index files
            assert indexes_exist(temp_ref, Mapper.BWA_MEM2), "BWA-MEM2 index should be created"
            
            # Index creation should complete in reasonable time
            assert index_time < 60, f"Index creation took too long: {index_time:.2f} seconds"
            
            # Check that all expected index files exist
            index_files = get_index_files(temp_ref, Mapper.BWA_MEM2)
            for index_file in index_files:
                assert index_file.exists(), f"Index file {index_file} should exist"
                assert index_file.stat().st_size > 0, f"Index file {index_file} should not be empty"
            
            print(f"BWA-MEM2 indexing completed in {index_time:.2f} seconds")
    
    def test_minimap2_mapping_real_data(self, real_reference_path, real_reads_r1_path, real_reads_r2_path):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert mapping_rate > 0.5, f"Mapping rate should be reasonable: {mapping_rate:.3f}"
            
            # Mapping should complete in reasonable time
            assert mapping_time < 120, f"Mapping took too long: {mapping_time:.2f} seconds"
            
            print(f"Minimap2 mapping completed in {mapping_time:.2f} seconds")
            print(f"Mapping rate: {mapping_rate:.3f}")
    
    def test_bwa_mem2_mapping_real_data(self, real_reference_path, real_reads_r1_path, real_reads_r2_path):
    ...
    def test_mapping_rate_calculation_realistic(self, real_reference_path, real_reads_r1_path, real_reads_r2_path):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert mapping_rate > 0.3, f"Real data should have reasonable mapping rate: {mapping_rate:.3f}"
            
            # Calculation should be fast
            assert calc_time < 10, f"Mapping rate calculation took too long: {calc_time:.2f} seconds"
            
            print(f"Mapping rate calculation: {mapping_rate:.3f} in {calc_time:.3f} seconds")
    
    def test_mapper_comparison_real_data(self, real_reference_path, real_reads_r1_path, real_reads_r2_path):
        ...
        for mapper_name, result in results.items():
            assert result['mapping_rate'] > 0.3, f"{mapper_name} should have decent mapping rate"
...
class TestMappingPerformance:
    ...
    def test_mapping_performance_scaling(self, small_reference_path):
        ...
        assert speedup_ratio < 3.0, f"Performance variance too high: {speedup_ratio:.2f}x"