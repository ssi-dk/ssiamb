...
class TestReuseRealData:
    ...
    def test_real_bam_contig_extraction(self, real_bam_path):
        ...
        assert any(len(name) > 5 for name in contigs.keys()), "Should have meaningful contig names"
    ...
    def test_real_fasta_contig_extraction(self, reference_fasta_path):
        ...
        assert total_length > 100000, f"Total FASTA length seems too small: {total_length}"
    ...
    def test_bam_fasta_compatibility_real_data(self, real_bam_path, reference_fasta_path):
    ...
    def test_vcf_fasta_compatibility_real_data(self, real_vcf_path, reference_fasta_path):
    ...
    def test_mismatched_sample_handling(self, mismatched_bam_path, reference_fasta_path):
    ...
    def test_multi_sample_vcf_handling(self, multi_sample_vcf_path, reference_fasta_path):
...
class TestReusePerformance:
    ...
    def test_large_bam_processing_performance(self, large_bam_path):
        ...
        assert process_time < 10, f"BAM processing took too long: {process_time:.2f} seconds"
        assert len(contigs) > 0, "Should extract contigs"
        
        print(f"Processed BAM with {len(contigs)} contigs in {process_time:.2f} seconds")
    ...
    def test_duplicate_detection_performance(self, large_bam_path):
        ...
        assert process_time < 10, f"Duplicate detection took too long: {process_time:.2f} seconds"
        assert isinstance(has_dups, bool)
