...
class TestMainCallback:
    ...
    def test_config_loading_success(self, mock_load_config):
        ...
        try:
            result = runner.invoke(app, ["--config", str(config_path), "--verbose", "self", "--r1", "test1.fq", "--r2", "test2.fq", "--assembly", "test.fa"])
    ...
    def test_config_loading_failure(self, mock_load_config):
        ...
        try:
            result = runner.invoke(app, ["--config", str(config_path), "self", "--r1", "test1.fq", "--r2", "test2.fq", "--assembly", "test.fa"])
...
class TestSelfCommand:
    ...
    def test_self_command_file_validation(self, mock_create_plan, mock_execute):
        ...
        result = runner.invoke(app, [
            "self",
            "--r1", "/nonexistent/r1.fastq",
            "--r2", "/nonexistent/r2.fastq", 
            "--assembly", "/nonexistent/assembly.fasta"
        ])
...
class TestCLIParameterValidation:
    ...
    @pytest.mark.parametrize("threads_value,expected_valid", [
        ("1", True),
        ("8", True),
        ("32", True),
        ("0", False),
    ...
    def test_threads_parameter_validation(self, threads_value, expected_valid):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('src.ssiamb.cli.create_run_plan'), patch('src.ssiamb.cli.execute_plan'):
                result = runner.invoke(app, [
                    "self",
                    "--r1", str(r1_path),
                    "--r2", str(r2_path),
                ...
                if expected_valid:
                    assert result.exit_code == 0 or "Missing option" not in result.stdout
                else:
                    assert result.exit_code != 0