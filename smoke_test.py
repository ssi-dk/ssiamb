...
def run_command(cmd: List[str], cwd: Optional[Path] = None, 
                timeout: int = 300, capture_output: bool = True) -> subprocess.CompletedProcess:
    """
    ...
    try:
        ...
        raise SmokeTestError(f"Command timed out after {timeout}s: {' '.join(map(str, cmd))}")
...
def verify_tsv_output(filepath: Path, expected_sample: Optional[str] = None) -> Dict[str, str]:
    """
    ...
    if len(lines) < 2:
        raise SmokeTestError(f"TSV file should have header + at least 1 data row, got {len(lines)} lines")
    ...
    if len(first_row) < len(header):
        raise SmokeTestError(f"First data row has {len(first_row)} columns, expected {len(header)}")
...
def test_admin_reference_setup(test_dir: Path) -> Path:
    ...
    try:
        ...
        if result:
            ...
            bwa_files = list(admin_ref_dir.glob(f"{test_species}.fna.bwa*")) + list(admin_ref_dir.glob(f"{test_species}.fna.*"))
            bwa_files = [f for f in bwa_files if f.suffix not in ['.fna'] and '.mmi' not in f.name]  # Exclude the FASTA file itself and .mmi
            if not bwa_files:
                raise SmokeTestError(f"No bwa-mem2 index files found for {test_species}")
    ...
    print(f"✓ Admin reference directory setup complete with reference for {test_species}")
    return admin_ref_dir
...
def test_self_mode_analysis(test_dir: Path) -> Path:
    ...
    cmd = [
        sys.executable, "-m", "ssiamb.cli", "self",
        "--r1", str(r1_file),
        "--r2", str(r2_file),
        "--assembly", str(assembly_file),
        "--sample", sample_name,
    ...
    result = run_command(cmd, timeout=600)  # Mapping can take time
...
def test_ref_mode_analysis(test_dir: Path, admin_ref_dir: Path) -> Path:
    ...
    cmd = [
        sys.executable, "-m", "ssiamb.cli", "ref",
        "--r1", str(r1_file),
        "--r2", str(r2_file),
        "--species", test_species.replace("_", " "),  # Convert to space format for CLI
        "--ref-dir", str(admin_ref_dir),
        "--sample", sample_name,
        "--outdir", str(output_dir),
        "--threads", "2",
        "--emit-vcf",
    ...
    vcf_files = list(output_dir.glob("*.vcf.gz"))
    matrix_files = list(output_dir.glob("*.matrix.tsv.gz")) or list(output_dir.glob("*matrix*.tsv.gz"))
...
def test_summarize_mode(test_dir: Path, self_output_dir: Path) -> None:
    ...
    # Find VCF and BAM files from self-mode output
    sample_name = "test_self_sample"
    ...
    # Run summarize mode
    cmd = [
        sys.executable, "-m", "ssiamb.cli", "summarize",
        "--vcf", str(vcf_file),
        "--bam", str(bam_file),
        "--output", str(output_dir / "summarize_result.tsv")
    ]
...
def test_error_handling(test_dir: Path) -> None:
    ...
    cmd = [
        sys.executable, "-m", "ssiamb.cli", "self",
        "--r1", "/nonexistent/file.fastq.gz",
        "--r2", "/nonexistent/file.fastq.gz", 
        "--assembly", "/nonexistent/assembly.fna",
        "--sample", "test_error"
    ]
    
    result = subprocess.run(cmd, capture_output=True, text=True)
    if result.returncode == 0:
        raise SmokeTestError("Expected failure for missing input files, but command succeeded")
    ...
    result = subprocess.run(cmd, capture_output=True, text=True)
    if result.returncode == 0:
        raise SmokeTestError("Expected failure for invalid species, but command succeeded")
...
def run_smoke_tests(clean_downloads: bool = False) -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            if admin_ref_dir:
                ref_output_dir = test_ref_mode_analysis(test_dir, admin_ref_dir)
            else:
